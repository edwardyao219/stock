...
def _should_exit(position: PaperPosition, bar, trade_date: date) -> tuple[bool, Decimal, str]:
    ...
    if position.current_stop is not None and low <= position.current_stop:
    ...
    if position.max_holding_days and _holding_days(position.entry_date, trade_date) >= position.max_holding_days:
        return True, close, "time_exit"

    return False, close, ""


def _open_position_from_plan(db, account, plan: TradePlan, trade_date: date, bar) -> tuple[bool, str]:
    ...
    return True, f"{plan.symbol} opened: {quantity} @ {entry_price}"
...
def run_daily_paper_simulation(
    ...
    with SessionLocal() as db:
        ...
        for position in load_open_positions(db, account.id):
            ...
            position.pnl_pct = (exit_price / position.entry_price - Decimal("1")).quantize(Decimal("0.000001"))
            closed += 1
        ...
        for plan in load_trade_plans_for_trade_date(db, current_date):
            ...
            if bar is None:
                ...
                continue
            ok, message = _open_position_from_plan(db, account, plan, current_date, bar)
            if ok:
                opened += 1
            else:
                skipped += 1
