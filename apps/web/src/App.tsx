...
function lateMarketTurnHealthText(value: Record<string, unknown> | undefined) {
...
function lateMarketTurnHealthDetail(value: Record<string, unknown> | undefined) {
  const coverage = value?.coverage_ratio;
  if (typeof coverage === "number") return `覆盖 ${(coverage * 100).toFixed(0)}%`;
                  ...
                  <div className={`after-close-push-status ${afterCloseStatusTone(afterCloseStatus?.status)}`}>
                    ...
                    {afterCloseStatus ? (
                      <div className="after-close-push-metrics">
                        ...
                        <span>筹码分布 {afterCloseStatus.cyq_perf_rows > 0 ? `${afterCloseStatus.cyq_perf_rows} 条` : "未到"} / {afterCloseStatusLabel(afterCloseStatus.cyq_perf_status)}</span>
                        ...
                        {Object.entries(afterCloseStatus.candidate_retire_reasons).map(([reason, count]) => <span key={reason}>淘汰 {count}：{reason}</span>)}
                        <span>资金更新 {dateTimeText(afterCloseStatus.moneyflow_updated_at)}</span>
                        <span>筹码更新 {dateTimeText(afterCloseStatus.cyq_perf_updated_at)}</span>
